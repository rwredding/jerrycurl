...
namespace Jerrycurl.Relations.Internal.V11.Parsing
{
    ...
    {
        public static NodeTree Parse(MetadataIdentity source, RelationIdentity2 relation)
        {
            NodeTree tree = new NodeTree();

            IRelationMetadata sourceMetadata = GetMetadata(source);
            IRelationMetadata[] relationMetadata = GetHeading(relation).ToArray();
        ...
        private static IEnumerable<IRelationMetadata> GetHeading(RelationIdentity2 relation)
            => relation.Heading.Select(GetMetadata);
